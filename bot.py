...
all_pages: Dict[str, MangaCard] = dict()
...
async def add_manga_options(chat_id,  output):
    ...
    if chat_options and chat_options.output == output:
        return None
    if chat_options:
        chat_options.output = output
    else:
        chat_options = MangaOutput(user_id=chat_id, output=output)
    try:
...
async def bot_ask(msg: Message, text: str, quote: bool = False, filters: filters = filters.text, timeout: int = 60):
    ...
    try:
        response = await bot.listen(msg.chat.id, filters=filters, timeout=timeout)
    except asyncio.TimeoutError:
        await request.edit_text("Process Timed Out! You were late in responding.")
        raise
    
    if response.text.lower().split(" ")[0] in ["/cancel"]:
        await request.edit("Process Cancelled!")
...
async def manga_click(client, callback: CallbackQuery, pagination: Pagination = None):
    ...
    if all_page_key not in all_pages:
        all_pages[all_page_key] = pagination.manga
        
        if await db.get_subs_by_url(pagination.manga.url):
            if not await db.get(LastChapter, pagination.manga.url):
                agen = pagination.manga.client.iter_chapters(pagination.manga.url, pagination.manga.name)
                last_chapter = await anext(agen)
                await db.add(LastChapter(url=pagination.manga.url, chapter_url=last_chapter.url))
...
async def chapter_click(client, data, chat_id, chapter=None, custom_caption=None, custom_filename=None, Id=None):
    ...
    async with locks[chat_id]:
        ...
        if download:
            ...
            thumbnail = await get_manga_thumb(chapter.manga)
            if thumbnail:
            ...
            if not custom_filename and not chapterFile:
...
async def all_page_click(client: Client, callback: CallbackQuery):
    manga = all_pages[callback.data]
    
    q, a = await bot_ask(callback.message, 'Send me the ID of the Chat where you want to upload this manga.')
    await q.delete()
    try:
        chat_id = int(a.text)
    except ValueError:
    ...
    try:
        tmp_msg = await bot.send_message(chat_id, manga.url)
        await tmp_msg.delete()
    except BaseException:
        await a.reply_text("Bot couldn't send a message to the provided chat ID. Make sure that bot is added correctly!")
        return
    
    q, a = await bot_ask(callback.message, 'Send me the file format for the chapters.\n\nYou can choose in ↓\n\n→<code>PDF</code>\n→<code>CBZ</code>\n→<code>BOTH</code>')
    await q.delete()
    output = file_options.get(a.text.lower(), None)
    if output is None:
    ...
    await add_manga_options(str(chat_id), output)
    
    details = f'<b>Site→</b> {manga.client.name}\n<b>Manga→</b> {manga.name}\n<b>Chat→</b> {chat_id}'
    status_message = await callback.message.reply_text('<i>Currently Uploading...</i>\n\n' + details, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Cancel', Id)]]))
    
    if Id not in bulk_process:
        bulk_process.append(Id)

    all_chapters = [chapter async for chapter in manga.client.iter_chapters(manga.url, manga.name)]
    for chapter in reversed(all_chapters):
        if Id not in bulk_process:
            await status_message.edit_text('<b>Upload Cancelled!</b>\n\n' + details, reply_markup=None)
            return
    ...
    await status_message.edit_text('<b>Upload Finished!</b>\n\n' + details, reply_markup=None)

async def bulk_process_click(client: Client, callback: CallbackQuery):
    bulk_process.remove(callback.data)
    await callback.answer("Upload will be cancelled soon!", show_alert=True)
...
async def on_callback_query(client, callback: CallbackQuery):
    ...
    elif callback.data in bulk_process:
        await bulk_process_click(client, callback)
